...
class CompositionProvider with ChangeNotifier {
  ...
  void filterCompositions(String query) {
    ...
    if (query.isEmpty) {
      ...
      List<String> queryParts = lowerQuery.split(' ').map((part) => part.trim()).toList();
      ...
      List<Composition> filteredCompositions = _compositions.where((composition) {
        ...
        bool matchesQuery = queryParts.every((part) =>
        lowerComposer.contains(part) || lowerTitle.contains(part) ||
            lastName.contains(part) || firstName.contains(part)
        );
